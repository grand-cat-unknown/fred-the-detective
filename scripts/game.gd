...
func _ready() -> void:
	...
	print("[case] loaded %d facts, %d interactables, and %d inventory items" % [case.fact_definitions.size(), case.interactables.size(), case.inventory_items.size()])
	...
	_inventory_panel.configure(_case_state, _world, case.inventory_items)
